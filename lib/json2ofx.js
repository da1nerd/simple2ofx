...
function makeOfxTrans(jsonTrans) {
    ...
    let categories = stringifyCategories(jsonTrans.categories);
    return {
        TRNTYPE: type,
        ...
        MEMO: categories + memo
    };
}

/***
 * Converts an array of categories into strings
 * @param jsonCategories
 */
function stringifyCategories(jsonCategories) {
    let str = '';
    if(jsonCategories) {
        for (let cat of jsonCategories) {
            str += `${cat['folder']}:${cat['name']}; `
        }
    }
    return str;
}
